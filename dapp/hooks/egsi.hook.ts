...
import EGSI_ABI from "../lib/egsi.abi.json";
...
export interface EGSIInterface {
    reload: () => void;
...
export function useEGSI(): EGSIInterface {
    ...
    const [estimatedReceivedTokens, setEstimatedReceivedTokens] = useState<BigNumber>();
    ...
    function createContract(): Contract {
        return new web3.eth.Contract(EGSI_ABI as any, egsiContractAddress);
    }
        ...
        return new BigNumber(
            web3.utils.fromWei(
                await contract.methods
                    .allowance(address, egsiContractAddress)
                    .call(),
                "ether"
            )
        );
    ...
    async function approveAmount(amount: string): Promise<void> {
        ...
        return contract.methods
            .approve(egsiContractAddress, web3.utils.toWei(amount, "ether"))
            .send({
                from: address,
            })
            .catch(errorHandler)
