...
contract Cronje {
    ...
    uint private constant MAX_SUPPLY= 3_000_000;
    uint private constant LP_FACTOR= 3;

    uint private constant MAX_PRICE= 100;
    ...
    constructor(Wigo _dex,string memory name, string memory symbol,address allowedTradeIn) {
        dex= _dex;
    ...
    function getToken(uint256 input) external {
        ...
        inputToken.transfer(address(DEAD),burn);
        burnedInput += burn;
